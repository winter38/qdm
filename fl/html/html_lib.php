...
function d_debug($val, $header = ''){ 
    ...
    if( isset($_SESSION['uid']) && $_SESSION['uid'] != 2 ) return true;
...
function d_block($val, $header, $hide = 1){

    if( $header === '' ) $header = 'Блок ';
    $style = '';

    echo '<div class="d_block_div">';
    echo '<div class="d_block_div_header plus">' . $header . '</div>';
    if( $hide ) $style = 'style="display: none;"';
    echo '<div class="d_block_div_body" '.$style.'>';
    echo($val);
    echo '</div>';
    echo '</div>';
    echo '<div style="clear: both"></div>';
...
function debug($val, $name = ''){ $_SESSION['d'][] = array($val, $name); }
...
function draw_debug(){
    ...
    if( isset($_SESSION['d']) ){
        ...
        $ci = count($_SESSION['d']);
        for( $i = 0; $i < $ci; $i++ ){ 
            d_debug($_SESSION['d'][$i][0], $_SESSION['d'][$i][1]);
        }
    }
}
...
function log_to_html($log, $mode = 0){
    ...
    for( $i=0; $i < $ci; $i++ ){ 
        ...
        for( $j=0; $j < $cj; $j++ ){
            ...
            if( !isset($cur['stat']['eva'])) $cur['stat']['eva'] = 0;
            // d_echo($cur);
            ...
            // HTML of one player
            $div = $cur['hp'] / $cur['hp_max'];
            $div = round($div * 100);
            if( $div < 0 ) $div = 0;
            
            $hp = '<span class="log_user_hp"><span class="log_user_hp_left">' . $cur['hp'] . '</span>/';
            $hp .= '<span class="log_user_hp_max">' . $cur['hp_max'] . '</span></span>';
            ...
            $res .= '</tr><tr>';
            $acc = 0;
            if( $cur['stat']['miss'] ) $acc  =  round( ($cur['stat']['hits'] / ($cur['stat']['miss'] + $cur['stat']['hits']))*100 );
            $crit_max = ( $cur['stat']['hits'] == 0 ) ? 1 : $cur['stat']['hits'];
            $crit =  round( ( ($cur['stat']['crit_count']) / $crit_max )*100 );
            $dodge = 0;

            if( $cur['stat']['defended'] ) $dodge = round( ( $cur['stat']['eva'] / $cur['stat']['defended']  )*100 );
    ...
    $html_grp .= '<div style="clear: both"></div>';
    $html .= $html_grp;
        ...
        for ($j=0; $j < $cj; $j++) { 
            ...
            if( $cur['action'] < 0 ){
                ...
                if( $cur['action'] == -1 ){
                    ...
                    $t_log .= '<td style="text-align:right">' . '[<span class="log_round_hp_left">' . $cur['p2_hp'] . '</span>/<span class="log_round_hp_max">' . $cur['p2_max_hp'] . '</span>]'; '</td>';
                ...
                // Grp bonus
                elseif( $cur['action'] == -2 ) $msg = msg_log_grp($log['header'], $cur);
            }
            else{
                continue;
            }
    ...
    for ($i=0; $i < $ci; $i++) { 
        $p = $players[$i];
        if( isset($_SESSION['uid']) && $p['id'] == $_SESSION['uid'] ){
...
function html_header(){
    ...
    <html>
      ...
      <link href="visual/add_style.css" type="text/css" rel="stylesheet">
      </head>
    <body>';
...
function html_arena($ajax = false){

    $res = '';
    $battles = qdm_vs_list();
    if( !$ajax ) $res = '<div class="ui_block ajax_arena">'; 
    ...
    $res .= '<div class="countdown"></div>';
    if( !$ajax ) $res .= $js;

    if( !$battles ){
        $res .= '<span class="arena_title">Арена пуста</span><div class="arena_div">';
        if( !qdm_vs_is_active($_SESSION['uid']) ) $res .= '<div class="bts">' . tpl_but_a($script, '', 'swords', 'Выжидать противника') . '</div>';
    }
    else{
...
function last_log(){
    ...
    $res = '<div class="ui_block"><span class="arena_title">История</span>';
    $res .= '<div class="arena_div">';
    $res .= '<div class="arena_content">';

    $log_detail = '';

    $attacks = qdm_log_history_by_player($_SESSION['uid']);
    ...
    if( !empty($attacks) ){
        ...
        for( $i = 0; $i < $ci; $i++ ){ 
            ...
            $atk .= '<div><b>' . date('d.m.Y H:i:s', $attacks[$i]['date']) . '</b>: На вас напали! <a href="history_log_by_id.php?id=' . $attacks[$i]['id'] . '">результат боя</a></div>';
        }
    }
    $res .= $atk; 

    if( isset($_SESSION['mining']) ){
        $ci = count($_SESSION['mining'])-1;
        $counter = $ci - 30;
        for( $i = $ci; $i > 0; $i-- ){ 
            
            if( $counter > 0 ){
                unset($_SESSION['mining'][$i]);
                $counter--;
            }
            else $log_detail .= '<div><b>' . $_SESSION['mining'][$i]['date'] . '</b>: ' . $_SESSION['mining'][$i]['msg'] . '</div>';
        }
        $_SESSION['mining'] = array_values($_SESSION['mining']);
        // unset($_SESSION['mining']);
    }
    $res .= $log_detail;
    ...
    if( isset($_SESSION['last_log']) ){ 
        ...
        debug($_SESSION['last_log'], 'last log');
        $res .= $log['res'];
        $res .= $log['t_log'];
        $res .= $log['stat'];
        $res .= $log['html'];
...
function skills_block(){
    ...
    $player = qdm_player($_SESSION['uid']);
    $skills = qdm_skills_can_learn($player);
    $act_skills = qdm_skills_act($_SESSION['uid']);
    $ci = count($act_skills);
    $act_ids = array();
    for( $i = 0; $i < $ci; $i++ ){
    ...
    for( $i = 0; $i < $ci; $i++ ){
        ...
        if( isset($skill['lvl']) ) $skill_level = ' [' . $skill['lvl'] . ']';
        else{ $skill_level = '';}
    ...
    for( $i = 0; $i < $ci; $i++ ){
        ...
        // Counter
        $time = $skill['act_ts'] - 7200; // WHY 1hour ofset???
...
function skills_act_block(){
    ...
    $skills = qdm_skills_act($_SESSION['uid'], 1);
    ...
    for( $i = 0; $i < $ci; $i++ ){
        ...
        $progress = '<div class="skill_exp"><div class="meter animate meter_'.$skill['group'].'">
...
function html_panel(){
    ...
    $items[] = check_menu_item('items', '?s=items', 'Инвентарь');
    $items[] = check_menu_item('shop',  '?s=shop',   'Магазин');
    $items[] = check_menu_item('quests', '?s=quests', 'Квесты');
    $items[] = check_menu_item('history', 'history.php', 'История');
    $items[] = check_menu_item('explore', '?s=explore', 'Исследовать');
