    ...
    for ($i = 0; $i < $ci; $i++) { 
        ...
        for ($j = 0; $j < $cj; $j++) {
            ...
            if( isset($cur['stat_name']) && $cur['stat_name'] ){
                ...
                else  $player[$cur['stat_name']] = (int)$cur['stat_value'];
            }

            if( isset($res[$cur['key_val_1']]) ) (int)$res[$cur['key_val_1']] += (int)$cur['val_1'];
            else $res[$cur['key_val_1']] = (int)$cur['val_1'];

            if( isset($res[$cur['key_val_2']]) ) (int)$res[$cur['key_val_2']] += (int)$cur['val_2'];
            else $res[$cur['key_val_2']] = (int)$cur['val_2'];

            if( isset($res[$cur['key_val_3']]) ) (int)$res[$cur['key_val_3']] += (int)$cur['val_3'];
            else $res[$cur['key_val_3']] = (int)$cur['val_3'];
...
function qdm_multiple_hit(&$p1, &$p2s, &$grp, $players, &$file, $log_type = 0){
    ...
    for( $i = 0; $i < $ci; $i++ ){ 
        ...
        if( isset($skills[S_SECOND_BREATH]) ){
            if( $skills[S_SECOND_BREATH]['count'] && ($p1['hp']/$p1['hp_max']) < 0.5 ){
...
function qdm_monster_init($player, $i, $grp){
